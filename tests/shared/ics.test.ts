...
// Die Testdaten stammen aus einem echten myTischtennis-Kalender des Basisprojekts.
// Der Vereinsname steht deshalb hier im Test — im Produktivcode kommt er aus den
// Vereinseinstellungen (club_aliases).
const CLUB_ALIASES = ['heiligenhaus', 'heiligenhauser'];
...
describe('ICS Parser tests', () => {
  ...
  it('should parse home and away games correctly based on team name', () => {
    ...
    const homeRes = determineHomeAway("Heiligenhauser SV III vs TV Klaswipper III", teamName, CLUB_ALIASES);
    expect(homeRes.isHome).toBe(true);
    expect(homeRes.opponent).toBe("TV Klaswipper III");

    const awayRes = determineHomeAway("VfL Engelskirchen vs Heiligenhauser SV III", teamName, CLUB_ALIASES);
